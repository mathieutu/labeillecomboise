...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const mapObject = ref()
    const markers = ref(
      distributeurs.map(distributeur => L.marker(distributeur.latLng, { icon })),
    )

    const findBestZoom = (markersToFocus) => {
      mapObject.value.fitBounds(L.featureGroup(markersToFocus).getBounds().pad(0.2), { animate: true })
    }

    onMounted(() => {
      mapObject.value = L.map(mapRef.value, {
        scrollWheelZoom: false,
      })

      // mapObject.value.setView(new L.LatLng(0, 0), 1)
      mapObject.value.addLayer(osmLayer)
      ...
      watch(
        ...
        clicked => {
          findBestZoom(clicked === null ? markers.value : [markers.value[clicked]])
        },
        { immediate: true },
      )
